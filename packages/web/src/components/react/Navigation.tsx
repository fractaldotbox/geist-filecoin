...
export function Navigation({ onSpacesClick }: NavigationProps) {
    ...
    const truncatedDisplayText = displayText.length > 20 ? `${displayText.substring(0, 17)}...` : displayText;
            ...
            <div className="flex justify-between items-center px-6 py-3">
                ...
                <div className="flex items-center gap-4">
                    <Button
                        ...
                        title={currentSpace ? `Current space: ${currentSpace.name}` : "Manage spaces"}
                    >
                        <Folder className="w-4 h-4" />
                        {currentSpace && <span className="w-2 h-2 bg-green-500 rounded-full" />}
