...
const TOKEN_TYPE_VALUES = ACCESS_SCHEMA.properties.anyOf.map((item: any) => item.tokenType);
...
function getClaimsOptions(tokenType: TokenType) {
    const found = ACCESS_SCHEMA.properties.anyOf.find((item: any) => item.tokenType === tokenType);
...
export default function AccessControlPanel() {
    ...
    const form = useForm<RuleFormType>({
        ...
        defaultValues: {
            ...
            ...Object.fromEntries(Object.keys(RULE_SCHEMAS[criteriaType].properties).map(k => [k, ""])),
    ...
    return (
        <div className="max-w-xl mx-auto py-10">
            <Card>
                <CardHeader>
                    <h2 className="text-xl font-bold">Create Access Rule</h2>
                </CardHeader>
                <CardContent>
                    <FormProvider {...form}>
                        <Form {...form}>
                                ...
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            ...
                                            <FormControl>
                                                <Select value={String(field.value)} onValueChange={val => {
                                            ...
                                            <FormItem>
                                                <div className="flex flex-col gap-2">
                                                    {getClaimsOptions(tokenType).map((claim: string) => {
                                                        ...
                                                        return (
                                                            <label key={claim} className="flex items-center gap-2">
                                                                <input
                                                                    type="checkbox"
                                                                    ...
                                                                    onChange={e => {
                                                                        if (e.target.checked) {
                                                                            ...
                                                                            field.onChange(value.filter((v: string) => v !== claim));
                                                                        }
                                                                    }}
                                                                />
                                                                <span>{claim}</span>
