...
test.describe('OAuth Unit Tests', () => {
  ...
  test('should handle OAuth mock scenarios', async ({ page }) => {
    ...
    await expect(page.locator('#error-msg')).toContainText('OAuth flow initiated');
  });

  test('should validate OAuth configuration', async ({ page }) => {
    // Test configuration validation
    await page.goto('data:text/html,<html><body><div id="config-test">OAuth Config Test</div></body></html>');
    ...
    await expect(page.locator('#config-test')).toContainText('OAuth Config Test');
  });
  ...
  test('should validate URL parameter handling', async ({ page }) => {
    ...
    const urlParams = await page.evaluate(() => {
      ...
      const mockUrl = 'http://localhost:5173/?bluesky_session=test-session-123&state=test-state';
