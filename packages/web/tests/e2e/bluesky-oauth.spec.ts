...
test.describe('Bluesky OAuth Flow', () => {
  ...
  test('should display Bluesky login button in OAuth tab', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: 'Continue with Bluesky' })).toBeVisible();
    
    // Check that the button contains the Bluesky icon (SVG)
    await expect(page.locator('button:has-text("Continue with Bluesky") svg')).toBeVisible();
  });

  test('should redirect to Bluesky OAuth when clicking Bluesky login button', async ({ page }) => {
    // Open login dialog
    await page.getByRole('button', { name: 'Login' }).click();
    ...
    await page.route('**/api/auth/bluesky/login*', async (route) => {
      ...
      await route.fulfill({
        status: 302,
        headers: {
          'Location': 'https://bsky.social/oauth/authorize?client_id=test&redirect_uri=test&state=test'
        }
      });
    });
  ...
  test('should handle OAuth callback and authenticate user', async ({ page }) => {
    ...
    await expect(page.locator('button:has([role="img"])').first()).toBeVisible();
  });
  ...
  test('should clean up URL parameters after successful OAuth', async ({ page }) => {
