...
export class AuthHelpers {
  ...
  async clickBlueskyLogin() {
    await this.page.getByRole('button', { name: 'Continue with Bluesky' }).click();
  }

  /**
   * Mocks a successful OAuth verification response
   */
  async mockSuccessfulOAuthVerification(options: {
    jwt?: string;
    did?: string;
    handle?: string;
  } = {}) {
   ...
   */
  async mockFailedOAuthVerification(error: string = 'Verification failed') {
    await this.page.route('**/api/auth/bluesky/verify', async (route) => {
      await route.fulfill({
        status: 401,
        contentType: 'application/json',
   ...
   */
  async mockOAuthRedirect(redirectUrl: string = 'https://bsky.social/oauth/authorize?client_id=test&redirect_uri=test&state=test') {
   ...
   */
  async completeOAuthFlow(sessionId: string = 'test-session-id') {
  ...
  async isUserAuthenticated() {
    return await this.page.locator('button:has([role="img"])').first().isVisible();
  }
