...
test.describe('Bluesky OAuth API Endpoints', () => {
  ...
  test.describe('OAuth Login Endpoint', () => {
    test('should redirect to Bluesky OAuth when accessing login endpoint', async ({ page }) => {
      // Visit the OAuth login endpoint directly
      const response = await page.goto(`${API_BASE_URL}/api/auth/bluesky/login`);
    ...
    test('should include handle parameter in OAuth URL when provided', async ({ page }) => {
      const response = await page.goto(`${API_BASE_URL}/api/auth/bluesky/login?handle=test.bsky.social`);
    ...
    test('should generate unique state parameter for each request', async ({ page }) => {
      const response1 = await page.goto(`${API_BASE_URL}/api/auth/bluesky/login`);
      const location1 = response1?.headers()?.location;
      
      const response2 = await page.goto(`${API_BASE_URL}/api/auth/bluesky/login`);
  ...
  test.describe('OAuth Callback Endpoint', () => {
    test('should handle missing code parameter', async ({ page }) => {
      const response = await page.goto(`${API_BASE_URL}/api/auth/bluesky/callback?state=test-state`);
    ...
    test('should handle missing state parameter', async ({ page }) => {
      const response = await page.goto(`${API_BASE_URL}/api/auth/bluesky/callback?code=test-code`);
    ...
    test('should handle OAuth error parameter', async ({ page }) => {
      const response = await page.goto(`${API_BASE_URL}/api/auth/bluesky/callback?error=access_denied&state=test-state`);
    ...
    test('should handle invalid state parameter', async ({ page }) => {
      const response = await page.goto(`${API_BASE_URL}/api/auth/bluesky/callback?code=test-code&state=invalid-state`);
  ...
  test.describe('OAuth Verify Endpoint', () => {
    test('should require sessionId parameter', async ({ page }) => {
      const response = await page.request.post(`${API_BASE_URL}/api/auth/bluesky/verify`, {
        data: {}
      });
  ...
  test.describe('CORS Configuration', () => {
    test('should handle CORS preflight requests', async ({ page }) => {
      const response = await page.request.fetch(`${API_BASE_URL}/api/auth/bluesky/verify`, {
        method: 'OPTIONS',
        headers: {
  ...
  test.describe('API Error Handling', () => {
    test('should return proper error format for server errors', async ({ page }) => {
